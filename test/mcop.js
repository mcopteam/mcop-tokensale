...
let MCOP_TOTAL_SUPPLY = 10000000000;
...
contract('MCOPCrowdSale', function(accounts) {
  it("MCOPCrowdSale start", async function () {
    let saleContract =  await MCOPCrowdSale.deployed();
    //console.log(saleContract);

    // let blance = await saleContract.getFuck.call();
    // console.log("balance: " + blance.valueOf());
    ...
    console.log("MCOPToken maxTotalSupply: " + helper.fromWei(maxTotalSupply).valueOf());
    ...
    assert.equal(helper.fromWei(presaleBalance),  MCOP_TOTAL_SUPPLY*PRE_SALE_STAKE/DIVISOR_STAKE, "presaleBalance is ok");

    let teamBalance = await tokenContract.balanceOf(accounts[4]);
    console.log("teamBalance: " + helper.fromWei(teamBalance).valueOf());
    assert.equal(helper.fromWei(teamBalance),  MCOP_TOTAL_SUPPLY*DEV_TEAM_STAKE/DIVISOR_STAKE, "teamBalance is ok");
    ...
    assert.equal(helper.fromWei(communityBalance),  MCOP_TOTAL_SUPPLY*COMMUNITY_STAKE/DIVISOR_STAKE, "communityBalance is ok");
    ...
    assert.equal(helper.fromWei(presaleBalance),  
      (MCOP_TOTAL_SUPPLY*PRE_SALE_STAKE/DIVISOR_STAKE-transferAmount), "lockBalance is ok after transfer");
  ...
  it('MCOPCrowSale test buy token', async function(){
      let now = Math.floor((new Date()).valueOf()/1000);
      let saleContract = await MCOPCrowdSale.new(accounts[0], accounts[1],accounts[2],accounts[3],accounts[4],accounts[5], now);
  ...
  it('MCOPCrowSale test release token', async function(){
    let now = (new Date()).valueOf()/1000;
    let saleContract = await MCOPCrowdSale.new(accounts[0], accounts[1],accounts[2],accounts[3],accounts[4],accounts[5], now);
    let address = await saleContract.mcopToken.call();
    console.log("MCOPCrowSale Create: mcopToken: " + address);
